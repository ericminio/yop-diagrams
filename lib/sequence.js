...
let computeColumnsPosition = (columns, input)=> {
    ...
    for (var i=0; i<columns.length; i++) {
        ...
        column.row = i==0? 0 : columns[i-1].row + columns[i-1].width
        if (i>0) {
            let delta = column.row - (columns[i-1].row+boxWidth(columns[i-1].actor.name, config) +config.margin)
            if (delta < 0) {
                column.row += -delta
            }
        }
    ...
    if (input.steps) {
        for (var y = 3, i=0; i<input.steps.length; i++) {
            ...
            columns.forEach((column)=>{
                diagram[y][column.row] = '|'
                lines.forEach((line, index)=>{
                    diagram[y+1+index][column.row] = '|'
                })                
                diagram[y+1+lines.length][column.row] = '|'
            })
            if (isLeftMessage(from, to)) {                                
                lines.forEach((line, index)=>{
                    writeLeft(line, diagram, from.row-1, y+1+index)
                })
                drawLeftArrow(diagram, from.row-1, to.row, y+1+lines.length)
            }
            else {
                lines.forEach((line, index)=>{
                    writeRight(line, diagram, from.row+1, y+1+index)
                })
                drawRightArrow(diagram, from.row+1, to.row, y+1+lines.length)
            }
            y += (1 + lines.length + 1)
        }
    }
