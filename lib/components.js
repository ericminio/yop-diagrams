...
let create = (input)=>{
    ...
    input.components.forEach((component)=>{
        ...
        while (current.reference !== undefined) {
            let parent = input.components.filter(c => c.name==current.reference)[0] 
    ...
    if (input.links) {
        input.links.forEach((link)=>{
            let origin = input.components.filter(c => c.name==link.origin)[0] 
            for (var i=0; i<link.path.length-1; i++) {
                let start = Object.assign({}, link.path[i])
                let end = Object.assign({}, link.path[i+1])
                start.row += origin.row
                start.column += origin.column
                end.row += origin.row
